# Cartes agent Zenith — Terra (bleu)

> Feuille de validation assemblée à partir des brouillons de décodage
> (`.superpowers/sdd/cards-decode-*.md`). **Rien n'est inventé** : ce document réorganise fidèlement
> le contenu des brouillons, il ne réinterprète pas les icônes.

**Couleur → planète** : violet = Mercure · orange = Vénus · bleu = **Terra** · rouge = Mars · vert/teal = Jupiter.

**Règle constante** : le 1er effet d'une carte est toujours `+1 influence sur la planète de la carte`.

**Légende confiance** : ✓ = lecture confiante (conforme au lexique confirmé) · ❓ = incertaine (hypothèse posée, à confirmer) · ❓❓ = très incertaine / icône non mappée (description littérale gardée).

**Légende type** : **OBLIGATOIRE** = icônes sans flèche de liaison (toujours appliqué) · **FACULTATIF** = action reliée par une flèche/chevron ▶ (y compris les conditions « ! » et les échelles de paliers) — le joueur choisit de l'activer ou non. Quand le brouillon lui-même ne tranche pas (ex. séparateur « / » non documenté), le Type est marqué ❓.

**Colonne « Réel »** : à remplir/corriger par l'utilisateur après vérification sur les cartes physiques.

18 cartes dans cette feuille (sur 90 au total, toutes planètes confondues) — **dont 3 cartes à planète non confirmée** (H3RB3RT, Sir Sam, Helena Kerr, cf. note ⚠️ ci-dessous).

> ⚠️ **Note sur 3 cartes (scans 23.02_46, _47, _49)** : le brouillon B6 signale que ce petit groupe de cartes a un
> bandeau **bleu-acier/gris**, visuellement distinct du bleu franc « Terra » des autres cartes, avec des noms de
> personnages « normaux » (H3RB3RT, Sir Sam, Helena Kerr) plutôt que des noms de dirigeants/savants,
> et des icônes d'effet **jamais vues ailleurs**. Le brouillon se demande si ces 15 cartes (46 à 60) forment un
> **sous-type de carte différent** (« Personnages/Célébrités » ou extension) avec ses propres règles, plutôt que
> de vraies cartes « agent planète ». Elles sont classées ici dans Terra par défaut (hypothèse la plus proche
> émise dans le brouillon), **à confirmer/reclasser avec l'utilisateur**.

---

### CHARLEMAGNE  (scan 22.44_1, coût 8, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris uni, flèche ↓) | OBLIG | ✓ | |
| 3 | +1 influence sur planète différente des précédentes (disque gris + « ≠ », flèche ↓) | OBLIG | ✓ | |
| 4 | +1 influence sur planète différente des précédentes (2e disque gris + « ≠ », flèche ↓) | OBLIG | ✓ | |

---

### V3RN3  (scan 22.44_13, coût 2, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Échelle : exiler 2 cartes → gagner 2 zénithium ; exiler 4 → 4 ; exiler 7 → 7 | FACUL | ✓ | |

---

### GILGAMESH  (scan 22.44_19, coût 9, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris sans chiffre) | OBLIG | ✓ | |
| 3 | +1 influence sur une planète au choix (2e disque gris sans chiffre, même bloc). À confirmer : deux gains indépendants ou un seul effet de +2 ? | OBLIG | ✓ | |

---

### BRUSSOLØ  (scan 22.44_22, coût 1, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Gagner 1 zénithium | OBLIG | ✓ | |

---

### Ice June  (scan 22.44_26, coût 4, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Exiler [une carte, quantité non chiffrée] → chevron ▶ → disque gris marqué « = » (signe égal, non documenté), flèche ↓ | FACUL | ❓❓ | |

---

### M4TH3SØN  (scan 22.44_27, coût 4, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | CONDITION : si je possède le badge Leader → +1 influence sur Terra | FACUL | ✓ | |

---

### Baron Goro  (scan 22.44_29, coût 4, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Disque gris avec flèche ROUGE vers le haut, petit disque bleu barré d'une diagonale accolé en bas à droite. Hypothèse : « planète Terra exclue » (influence au choix sauf Terra) | OBLIG | ❓❓ | |
| 3 | Gagner 3 zénithium (même bloc que l'effet 2) | OBLIG | ✓ | |

---

### ELISABETH  (scan 22.44_30, coût 2, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Exiler [une carte, quantité non chiffrée, probablement 1] + gagner 1 zénithium (icône carte fragmentée puis hexagone jaune « 1 »). Origine de la carte exilée (ma main / adversaire / pioche) non précisée | OBLIG | ❓ | |

---

### AUGUSTUS  (scan 23.02_1, coût 10, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris) | OBLIG | ✓ | |
| 3 | +1 influence sur une planète au choix (2e disque gris, même bloc) | OBLIG | ✓ | |
| 4 | +1 influence sur une planète au choix (3e disque gris, même bloc). À confirmer : 3 gains distincts ou un seul « +3 » ? | OBLIG | ✓ | |

---

### Sneaky Jules  (scan 23.02_23, coût 1, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Gagner 4 crédits | OBLIG | ✓ | |
| 3 | Payer 1 zénithium (flèche rouge ↑) → chevron ▶ → gagner 2 influence sur une même planète au choix (disque gris, double flèche ↓↓ — conforme au lexique) | FACUL | ❓ (coût) / ✓ (récompense) | |

---

### L0V3CR4FT  (scan 23.02_25, coût 5, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Exiler 1 carte Mercure → chevron ▶ → gagner 1 zénithium | FACUL | ✓ | |
| 3 | Exiler 1 carte Vénus → chevron ▶ → gagner 1 zénithium | FACUL | ✓ | |
| 4 | Exiler 1 carte Mars → chevron ▶ → gagner 1 zénithium | FACUL | ✓ | |
| 5 | Exiler 1 carte Jupiter → chevron ▶ → gagner 1 zénithium | FACUL | ✓ | |

*Note : cette carte sert de référence — elle confirme que l'icône « chevron/carte déchirée coloré » = exiler 1 carte de cette planète (utile pour réinterpréter Chaka et Stessy Power, cf. `cartes-questions.md`).*

---

### ZENON  (scan 23.02_43, coût 1, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Gagner 8 crédits | OBLIG | ✓ | |
| 3 | Silhouette blanche (« chez l'adversaire ») + chevron, suivie de gagner 2 crédits (SANS flèche ▶ de liaison entre les blocs). Rôle exact de l'adversaire entre les deux gains (8 puis 2) incertain — vol non signalé par une pastille rouge | OBLIG | ❓❓ | |

---

### Lord Creep  (scan 23.02_44, coût 5, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris uni, flèche ↓) | OBLIG | ✓ | |
| 3 | Prendre le badge Leader (badge bicolore, forme de main, sans « ! » ni chevron) | OBLIG | ❓ | |

---

### H3RB3RT  (scan 23.02_46, coût 6, peuple Robot) — ⚠️ planète non confirmée
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur la planète du bandeau (bleu-acier, ❓ Terra) | OBLIG | ✓ (sous réserve couleur) | |
| 2 | 4 jetons en forme de fanion colorés (violet, orange, rouge, vert-teal), chacun relié par sa propre flèche ↓ à un disque plein de la même couleur juste en dessous. Hypothèse : +1 influence sur chacune des 4 planètes AUTRES que celle du bandeau (aucun chevron de choix entre les 4 jetons) | OBLIG | ❓❓ | |

---

### Sir Sam  (scan 23.02_47, coût 7, peuple Humain) — ⚠️ planète non confirmée
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur la planète du bandeau (bleu-acier, ❓ Terra) | OBLIG | ✓ (sous réserve couleur) | |
| 2 | Disque gris traversé d'une barre horizontale noire, flèche ↓, sans chiffre. Hypothèse prudente : gain nul/« rien » (carte volontairement faible), ou symbole non documenté | OBLIG | ❓❓ | |

---

### Helena Kerr  (scan 23.02_49, coût 6, peuple Humain) — ⚠️ planète non confirmée
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur la planète du bandeau (bleu-acier, ❓ Terra) | OBLIG | ✓ (sous réserve couleur) | |
| 2 | Disque brun/gris surmonté d'un petit cercle barré d'une diagonale (panneau d'interdiction), flèche ↓ en dessous. Même icône que la carte Suleiman (Jupiter, cf. `cartes-questions.md`) | OBLIG | ❓❓ | |

---

### F4RM3R  (scan 23.02_51, coût 6, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris, flèche ↓) | OBLIG | ✓ | |
| 3 | +1 influence sur planète différente des précédentes (disque gris « ≠ », flèche ↓) | OBLIG | ✓ | |

---

### TIBERIUS  (scan 23.02_54, coût 3, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Terra | OBLIG | ✓ | |
| 2 | Échelle : exiler 2 cartes → gagner 1 influence sur Terra ; exiler 4 → 2 ; exiler 7 → 3 | FACUL | ❓ (structure ✓, nature des cartes exilées à confirmer) | |

---

## Récapitulatif — 18 cartes Terra
Charlemagne, V3rn3, Gilgamesh, Brussoløc, Ice June, Mathesøn, Baron Goro, Elisabeth, Augustus, Sneaky Jules, L0v3cr4ft, Zenon, Lord Creep, F4rm3r, Tiberius (15 cartes de planète confirmée) + H3rb3rt, Sir Sam, Helena Kerr (3 cartes de planète ⚠️ non confirmée, classées ici par défaut).

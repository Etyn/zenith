# Cartes agent Zenith — Mercure (violet)

> Feuille de validation assemblée à partir des brouillons de décodage
> (`.superpowers/sdd/cards-decode-*.md`). **Rien n'est inventé** : ce document réorganise fidèlement
> le contenu des brouillons, il ne réinterprète pas les icônes.

**Couleur → planète** : violet = **Mercure** · orange = Vénus · bleu = Terra · rouge = Mars · vert/teal = Jupiter.

**Règle constante** : le 1er effet d'une carte est toujours `+1 influence sur la planète de la carte`.

**Légende confiance** : ✓ = lecture confiante (conforme au lexique confirmé) · ❓ = incertaine (hypothèse posée, à confirmer) · ❓❓ = très incertaine / icône non mappée (description littérale gardée).

**Légende type** : **OBLIGATOIRE** = icônes sans flèche de liaison (toujours appliqué) · **FACULTATIF** = action reliée par une flèche/chevron ▶ (y compris les conditions « ! » et les échelles de paliers) — le joueur choisit de l'activer ou non. Quand le brouillon lui-même ne tranche pas (ex. séparateur « / » non documenté), le Type est marqué ❓.

**Colonne « Réel »** : à remplir/corriger par l'utilisateur après vérification sur les cartes physiques.

18 cartes dans cette feuille (sur 90 au total, toutes planètes confondues).

---

### Guy Gambler  (scan 22.44_2, coût 3, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Gagner 5 crédits | OBLIG | ✓ | |
| 3 | CONDITION : si je possède le badge Leader (argent ou or, peu importe) → gagner un jeton bonus (pastille dorée à étoile) | FACUL | ✓ | |

---

### NERO  (scan 22.44_3, coût 7, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Gagner 3 zénithium | OBLIG | ✓ | |

---

### ORWELL  (scan 22.44_4, coût 1, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Gagner un jeton bonus (pastille dorée à étoile, sans flèche) | OBLIG | ✓ | |
| 3 | FACULTATIF : donner le badge Leader à l'adversaire (flèche rouge ↑ + badge, lexique confirmé) → chevron ▶ → gagner 7 crédits | FACUL | ✓ | |

---

### HUXLEY  (scan 22.44_8, coût 5, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris/olive, flèche ↓, sans chiffre) | OBLIG | ✓ | |
| 3 | CONDITION : si je possède le badge Leader → gagner 3 crédits | FACUL | ✓ | |

---

### Master Din  (scan 22.44_16, coût 6, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | +1 influence sur Mars (disque rouge, flèche ↓, sans chiffre) | OBLIG | ✓ | |
| 3 | Prendre le badge Leader (icône ailée bicolore, sans « ! » — lu comme action inconditionnelle) | OBLIG | ✓ | |

---

### ATLAS  (scan 22.44_18, coût 3, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Exiler (icône carte fendue/fragmentée, sombre, sans chiffre visible → probablement 1 carte) | OBLIG | ✓ | |
| 3 | Prendre le badge Leader (sans « ! » — action inconditionnelle) | OBLIG | ✓ | |

---

### W3LLS  (scan 22.44_20, coût 4, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | CONDITION : si je possède le badge Leader (argent ou or) → +1 influence sur Mercure | FACUL | ✓ | |

---

### KHAN  (scan 22.44_21, coût 9, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris sans chiffre) | OBLIG | ✓ | |
| 3 | FACULTATIF : donner le badge Leader à l'adversaire (flèche rouge ↑ + badge, lexique confirmé) | FACUL | ✓ | |
| 4 | → chevron ▶ → +1 influence sur planète différente des précédentes (disque gris « ≠ »). Question : le chevron relie-t-il un simple enchaînement ou une dépendance conditionnelle à la prise du badge ? | FACUL | ✓ | |

---

### WUL  (scan 23.02_4, coût 2, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | FACULTATIF : exiler 2/4/7 cartes Mercure (violet, couleur de la carte) → gagner 2/4/7 zénithium (échelle confirmée B2, ratio 1:1) | FACUL | ✓ | |

---

### Secret Kali  (scan 23.02_6, coût 2, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | FACULTATIF : donner 3 crédits à l'adversaire (flèche rouge ↑, B1) → chevron ▶ → +1 influence sur une planète ≠ Mercure (disque gris barré, B3 — couleur exclue présumée = celle de la carte) | FACUL | ✓ | |

---

### H4MILTØN  (scan 23.02_9, coût 2, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Mobiliser x2 (deux icônes « + » côte à côte) | OBLIG | ✓ | |
| 3 | FACULTATIF : donner le badge Leader à l'adversaire (flèche rouge ↑ + badge, lexique confirmé) → chevron ▶ → mobiliser x3 (trois icônes « + » empilées) | FACUL | ✓ | |

---

### Double Joe  (scan 23.02_13, coût 5, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Gagner 2 zénithium | OBLIG | ✓ | |
| 3 | FACULTATIF : donner le badge Leader à l'adversaire (flèche rouge ↑ + badge, lexique confirmé) → chevron ▶ → gagner 2 zénithium | FACUL | ✓ | |

---

### CHAKA  (scan 23.02_16, coût 1, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | FACULTATIF : exiler 2 cartes ≠ Mercure (violet) → gagner 10 crédits (palier unique — exemple explicitement cité dans la grammaire confirmée B3) | FACUL | ✓ | |

---

### MAGELLAN  (scan 23.02_19, coût 10, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris, flèche ↓) | OBLIG | ✓ | |
| 3 | +1 influence sur planète différente des précédentes (disque gris + « ≠ », flèche ↓) | OBLIG | ✓ | |
| 4 | +1 influence sur chacune des 5 planètes (rangée de 5 disques, un par planète — atome influenceEach, confirmé) | OBLIG | ✓ | |

---

### Lula Smart  (scan 23.02_27, coût 1, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | Gagner 4 crédits | OBLIG | ✓ | |
| 3 | FACULTATIF : donner 1 zénithium à l'adversaire (flèche rouge ↑, B1) → chevron ▶ → +1 influence sur Terra | FACUL | ✓ | |

---

### AMYTIS  (scan 23.02_34, coût 3, peuple Animod)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | FACULTATIF : exiler 2/4/7 cartes Mercure (violet, couleur de la carte) → chevron ▶ → gagner 1/2/3 influence sur Mercure (échelle confirmée B2) | FACUL | ✓ | |

---

### CL4RKE  (scan 23.02_36, coût 9, peuple Robot)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | +1 influence sur une planète au choix (disque gris/noir uni, flèche ↓) | OBLIG | ✓ | |
| 3 | FACULTATIF : donner 1 zénithium à l'adversaire (flèche rouge ↑, B1) → chevron ▶ → +1 influence sur planète différente des précédentes (disque gris « ≠ », icône déjà connue) | FACUL | ✓ | |

---

### Punk Mari  (scan 23.40_02, coût 7, peuple Humain)
| # | Effet décodé | Type | Conf. | Réel (à confirmer/corriger) |
|---|---|---|---|---|
| 1 | +1 influence sur Mercure | OBLIG | ✓ | |
| 2 | (Compartiment supérieur : tuiles en losange = **ILLUSTRATION**, aucun effet de jeu — confirmé utilisateur) | — | ✓ | |
| 3 | Défausser toute sa main (icône « main tenant un éventail de cartes » = discardHandAll, confirmé) → +1 influence sur une planète au choix (disque gris uni, flèche ↓) | OBLIG | ✓ | |

---

## Récapitulatif — 18 cartes Mercure
Guy Gambler, Nero, Orwell, Huxley, Master Din, Atlas, W3lls, Khan, Wul, Secret Kali, H4milton, Double Joe, Chaka, Magellan, Lula Smart, Amytis, Cl4rke, Punk Mari.
